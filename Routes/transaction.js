...
router.post("/withdraw/request", Auth, async (req, res) => {
    try {
        ...
        if (pendingGame.length == 0) {
            if (((parseInt(user.lastWitdrawl) + 3600000) < currentTime && lasttrans.status=='SUCCESS') || (user.lastWitdrawl == null) || (!lasttrans || lasttrans.status!='SUCCESS')) {
        ...
        if(user.withdraw_holdbalance>0 && type=='upi')
        {
                ...
                (
                    async () => {
                        ...
                        data: 
                        {
                            "account_number": "4564560186107152",
                            "amount": amount*100,
                            "currency": "INR",
                            "mode": "UPI",
                            "purpose": "payout",
